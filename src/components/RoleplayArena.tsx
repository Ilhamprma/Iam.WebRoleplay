...
export const RoleplayArena: React.FC<RoleplayArenaProps> = ({
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] max-w-5xl mx-auto w-full px-3 md:px-6 py-3">
      {/* Top Scene Context Banner */}
      <div className="flex items-center justify-between px-3.5 py-2 mb-2.5 rounded-xl bg-white dark:bg-[#0c0e15] border border-slate-200 dark:border-white/[0.08] shadow-xs text-xs">
        <div className="flex items-center gap-2 overflow-hidden">
          <span className="font-mono text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300 font-bold shrink-0">
            Lokasi
          </span>
        ...
        <button
          onClick={onOpenCodex}
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/[0.06] hover:bg-slate-200 dark:hover:bg-white/[0.12] border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-200 font-semibold transition shrink-0"
        >
          <BookOpen className="w-3.5 h-3.5" />
          <span>Codex & Inventaris</span>
        </button>
      </div>

      {/* Main Narrative Chronicle Feed */}
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 p-4 rounded-2xl bg-white dark:bg-[#090b10] border border-slate-200 dark:border-white/[0.06] shadow-xs">
        {campaign.roleplayMessages.map((msg, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`max-w-3xl rounded-2xl p-4 text-xs md:text-sm leading-relaxed border transition relative ${
                  isUser
              ...
              >
                {editingMessageId === msg.id ? (
                  ...
                  <>
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition flex items-center gap-1 bg-white/90 dark:bg-black/70 backdrop-blur-xs p-1 rounded-md border border-slate-200 dark:border-white/10 shadow-xs">
                      <button
                        onClick={() => {
        ...
        {error && (
          <div className="p-3.5 rounded-xl bg-red-500/10 border border-red-500/25 text-red-600 dark:text-red-400 text-xs space-y-2">
      ...
      {/* Action Modifiers & Tools */}
      <div className="flex items-center justify-between gap-2 my-2 text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            type="button"
            onClick={() => insertActionModifier('action')}
            className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-white dark:bg-white/[0.03] hover:bg-slate-100 dark:hover:bg-white/[0.08] border border-slate-200 dark:border-white/[0.06] text-slate-700 dark:text-slate-300 font-mono text-[11px] transition shadow-xs"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-white dark:bg-white/[0.03] hover:bg-slate-100 dark:hover:bg-white/[0.08] border border-slate-200 dark:border-white/[0.06] text-slate-500 dark:text-slate-400 text-[11px] transition shadow-xs"
          >
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-white dark:bg-white/[0.03] hover:bg-slate-100 dark:hover:bg-white/[0.08] border border-slate-200 dark:border-white/[0.06] text-slate-800 dark:text-slate-200 font-mono text-[11px] transition shadow-xs"
        >
          <Dices className="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" />
          Roll D20
        </button>
      </div>
